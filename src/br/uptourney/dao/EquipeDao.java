...
public class EquipeDao {
	...
	public boolean cadastrar(Equipe equipe, Usuario usuario) {
		try {
			PreparedStatement pstm = conexao
					.prepareStatement("insert into equipes (nome_equipe, id_jogo, id_user_lider) values (?,?,?)");
			...
			PreparedStatement pstm4 = conexao
					.prepareStatement("select id_usuario from usuarios where login = ? and senha = ?");
	...
	public Equipe listarEquipeID(long codigo) {
		Equipe equipe = null;
		try {
			// PASSO 2
			...
			// PASSO 4
			if (rs.next()) {
				//Jogo jogo = jogoDAO.getByID(rs.getLong("id_usuario"));
			...
			System.out.println("Deu pau aqui1zxc2312");
		}
		return equipe;
